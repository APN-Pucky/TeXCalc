...
public class CellList {
	...
	/*
	 * @JsonIgnore public String FRAMETOP = "\\usepackage{amsfonts}\n"+
	 * "\\usepackage{amsmath}\n" + "\\usepackage{amsthm}\n"+
	 * "\\usepackage{slashed}"+ "\\usepackage[compat=1.1.0]{tikz-feynman}\n" +
	 * "\\DeclareMathOperator{\\Tr}{Tr}\n"+ "\\setlength\\parindent{0pt}\n"+
	 * "\\begin{document}\n";
	 * 
	 * @JsonIgnore public String FRAMEEND = "\\end{document}\n";
	 */
	...
	public void linkAll() {
		for (int i = 0; i < cells.size(); ++i) {
			//GUI.log.d("" + i + "/" + cells.size(), "celllist");
			unlink(i);
			link(i);
		}
	}
	...
	public void link(int index) {
		...
		vg.addGroup(p);
		p.addComponent(cell.panel);
		p.addComponent(cell.icon);
		hg1.addComponent(cell.panel);
		hg2.addComponent(cell.icon);

		panel.add(cell.panel);
		panel.add(cell.icon);
	...
	public synchronized void increase(int index) {
		Cell c = new Cell(latex);
		cells.add(index, c);
		// link(cells.size()-1);
		linkAll();
	...
	public synchronized void remove(Cell c) {
		unlink(c);
		cells.remove(cells.indexOf(c));
		re();
	}

	/*
	 * public void decrease() { Cell c = cells.get(cells.size()-1);
	 * unlink(cells.size()-1); cells.remove(c); }
	 */
