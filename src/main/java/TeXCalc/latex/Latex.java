...
public class Latex {
	...
	public void setStandaloneType(String a) {
		standaloneType.setText(a);
	}

	public void setDocumentType(String a) {
		documentType.setText(a);
	}
	...
	public Latex() {
		this(FRAMETOP, FRAMEEND, TYPE_STANDALONE, TYPE_DOCUMENT);
	}

	@JsonCreator
	public Latex(@JsonProperty("top") String top, @JsonProperty("end") String end,
			@JsonProperty("standalone") String standalone, @JsonProperty("document") String document) {
		panel = new JPanel();

		this.top = GUI.areaLatex(top);
		this.end = GUI.areaLatex(end);
		engine = GUI.area(Config.current.getLatex().getDefaultEngine().getValue());
		standaloneType = GUI.areaLatex(standalone);
		documentType = GUI.areaLatex(document);
	...
	public void pasteCache(String dir) {
		if (!new File(dir).isDirectory())
			throw new RuntimeException("Can only paste Cache in directory");
		for (String f : filecache.keySet()) {
			...
			if (ff.exists()) {
				...
				byte[] decodedImg = Base64.getDecoder().decode(filecache.get(f).getBytes(StandardCharsets.UTF_8));
				try {
	...
	/*
	 * public File toPdf2(String latex) { String uuid =
	 * UUID.randomUUID().toString(); String TEMP_DIRECTORY = ".tmp" + File.separator
	 * + uuid; String TEMP_TEX_FILE_NAME = "export"; // for New22.tex String ret =
	 * TEMP_DIRECTORY + File.separator + TEMP_TEX_FILE_NAME + ".png"; File ret_file
	 * = new File(TEMP_DIRECTORY + File.separator + TEMP_TEX_FILE_NAME + ".pdf") ;
	 * 
	 * // 1. Prepare the .tex file String newLineWithSeparation =
	 * System.getProperty("line.separator") + System.getProperty("line.separator");
	 * String math = latex;
	 * 
	 * System.out.println(" 2. Create the .tex file"); FileWriter writer = null; try
	 * { File dir = new File(TEMP_DIRECTORY + File.separator + "tex"); if
	 * (!dir.exists()) dir.mkdirs(); writer = new FileWriter( TEMP_DIRECTORY +
	 * File.separator + "tex" + File.separator + TEMP_TEX_FILE_NAME + ".tex",
	 * false); writer.write(math, 0, math.length()); writer.close(); } catch
	 * (IOException ex) { ex.printStackTrace(); }
	 * 
	 * for(String f : filecache.keySet()) { File ff = new File(f); if(ff.exists()) {
	 * try { System.out.println("copied"); File fi = (new File(TEMP_DIRECTORY +
	 * File.separator + "tex" + File.separator+ f)); fi.getParentFile().mkdirs();
	 * Files.copy(ff,fi); } catch (IOException e) { // TODO Auto-generated catch
	 * block e.printStackTrace(); } cache(f);
	 * 
	 * } else { byte[] decodedImg = Base64.getDecoder()
	 * .decode(filecache.get(f).getBytes(StandardCharsets.UTF_8)); try { File fn =
	 * new File (TEMP_DIRECTORY + File.separator + "tex" + File.separator + f);
	 * fn.getParentFile().mkdirs(); fn.createNewFile(); FileOutputStream fos = new
	 * FileOutputStream(fn); fos.write(decodedImg); fos.close(); } catch
	 * (IOException e) { // TODO Auto-generated catch block e.printStackTrace(); } }
	 * }
	 * 
	 * System.out.print("  3. Execute LaTeX {" + uuid +
	 * "} from command line  to generate picture = "); ProcessBuilder pb = new
	 * ProcessBuilder(getEngine(), "-halt-on-error", TEMP_TEX_FILE_NAME + ".tex");
	 * pb.directory(new File(TEMP_DIRECTORY + File.separator + "tex")); try { long
	 * startTime = System.nanoTime(); Process p = pb.start(); if(PRINT) {
	 ...
	 * Task.startUntracked(fluxSortie); Task.startUntracked(fluxErreur); }
	 * p.waitFor(); long stopTime = System.nanoTime();
	 * if(TIME)System.out.println((stopTime - startTime) / 1.e9 + " s"); } catch
	 * (IOException | InterruptedException ex) { ex.printStackTrace(); }
	 * 
	 * try { File tmp = new File(TEMP_DIRECTORY + File.separator + "tex" +
	 * File.separator + TEMP_TEX_FILE_NAME + ".pdf"); if(tmp.exists()) {
	 * Files.move(tmp,new File(TEMP_DIRECTORY + File.separator + TEMP_TEX_FILE_NAME
	 * + ".pdf") ); } else { ret_file = null; } } catch (IOException e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); ret_file = null; }
	 * 
	...
	public BufferedImage pdfToImage(File pdf_file) {
		...
		if (f == null)
			return null;
		ProcessBuilder pb = new ProcessBuilder("pdftoppm", "-png", f.getAbsolutePath());
	...
	private void cleanUp(String TEMP_DIRECTORY, String TEMP_TEX_FILE_NAME) {
		...
		/*
		 * for (File file : (new File(TEMP_DIRECTORY + File.separator +
		 * "tex").listFiles())) { if (file.getName().startsWith(TEMP_TEX_FILE_NAME +
		 * ".")) { file.deleteOnExit(); } }
		 */
	}
